...
object XtreamStreamUrlFactory {
    ...
    /**
     * The live format for a channel whose listing is not loaded.
     *
     * A channel played from the saved list or the guide is played by id alone: the category it came
     * from was never fetched, so its own container is unknown and only what the account advertises
     * remains. This is exactly the answer [selectFormat] gives in that case, kept as its own function
     * so the caller does not have to invent a [LiveChannel] to ask the question.
     */
    fun selectFormat(account: Account): String =
        if (account.allowedOutputFormats.any { it.lowercase() == "m3u8" }) "m3u8" else "ts"
