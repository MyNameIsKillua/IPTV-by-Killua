...
data class Account(
    ...
    /**
     * Where this account's library comes from.
     *
     * Carried on the account rather than inferred from the empty user name a playlist has, because
     * the screens need it and "no user name means a playlist" is the sort of rule that stops being
     * true without anything failing to compile.
     */
    val source: LibrarySource = LibrarySource.Xtream,
) {
    ...
    /**
     * True where the library is a playlist file, which has channels and nothing else.
     *
     * An M3U has no films, no series and no guide endpoint, so the screens for those are removed
     * rather than shown empty: an empty library reads as one that failed to load, and sends someone
     * looking for a fault that is not there.
     */
    val isPlaylist: Boolean get() = source == LibrarySource.Playlist
}
...
data class LiveChannel(
    ...
    val streamHeaders: StreamHeaders? = null,
)

/**
 * Request headers a playlist says a channel needs, and without which its server answers 403.
 *
 * This exists only because playlists carry it; an Xtream channel never has one, since the account
 * is the authorisation and `XtreamStreamUrlFactory` builds the address. Measured against iptv-org's
 * `index.m3u` on 24 August 2026: of 12,785 channels, **641 name a user agent and 198 a referrer,
 * 785 naming at least one** - one channel in sixteen, which is too many to drop.
 *
 * A playlist states them twice, in two syntaxes: as attributes on the `#EXTINF` line, and as
 * `#EXTVLCOPT:` directives beneath it. Both are read, and the attribute wins where they disagree,
 * for the same reason `group-title` wins over `#EXTGRP` - the attribute is the modern spelling and
 * the directive is what older players understood. Counting occurrences of `http-user-agent=` in the
 * file gives roughly twice the number of channels for exactly that reason, which is a trap worth
 * knowing before quoting a figure from `grep`.
 *
 * These are hints from an untrusted file and are treated as such: bounded in length, stripped of
 * control characters, and never merged into a request as raw header text.
 */
data class StreamHeaders(
    val userAgent: String? = null,
    val referrer: String? = null,
) {
    val isEmpty: Boolean get() = userAgent == null && referrer == null
}
...
class XtreamCredentials(
    ...
    val password: String,
    /**
     * Where this account's library comes from. See [LibrarySource].
     *
     * Defaulted, so every existing caller keeps meaning what it meant: an Xtream account.
     */
    val source: LibrarySource = LibrarySource.Xtream,
) {
...
/**
 * The two kinds of library this program can be pointed at.
 *
 * [Playlist] puts the playlist's own address in [XtreamCredentials.serverUrl] and leaves the user
 * name and password empty, because a playlist has neither - it is a file, and reading it is the
 * whole of the authorisation.
 *
 * It is written down rather than inferred from those empty fields. "No user name means a playlist"
 * would be true today and is exactly the kind of rule that stops being true quietly; a value that
 * says what it is cannot drift.
 */
